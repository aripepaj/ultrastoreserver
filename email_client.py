...
MAIL_FROM = os.getenv("MAIL_FROM", "no-reply@ultrastore-ks.com")        # must be verified or domain-auth’d
MAIL_FROM_NAME = os.getenv("MAIL_FROM_NAME", "Ultrastore Orders")
MAIL_BCC = os.getenv("MAIL_BCC")  # optional
REPLY_TO = os.getenv("REPLY_TO")  # optional (e.g., orders@ultrastore-ks.com)

def send_email(to_emails, subject, body_text, body_html=None, order_id="UNKNOWN"):
    if not SENDGRID_API_KEY:
        log_failed_order(order_id, "Missing SENDGRID_API_KEY for email")
        return {"error": "Email not configured"}

    # de-duplicate TO (case-insensitive)
