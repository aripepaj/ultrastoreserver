...
from utilis import (
    ...
    extract_phone,
    extract_address,
    normalize_phone_number_digits_only,  # <- strips + and non-digits
    variant_text,                        # <- builds Size/Color/... text
    money,                               # <- formats money with currency
)
...
ALWAYS_EMAIL = os.getenv("ALWAYS_EMAIL", "true").lower() == "true"  # email even if cheetah fails (helpful)
SHOPIFY_WEBHOOK_SECRET = os.getenv("SHOPIFY_WEBHOOK_SECRET", "")

city_to_id = {
    "prishtina": 1, "prishtine": 1,  # alias added
    ...
    "peje": 16, "pejë": 16, "podujeve": 17, "podujevë": 17, "prizren": 18, "skenderaj": 19,
    "skënderaj": 19, "shtime": 20, "shterpce": 21, "shtërpcë": 21, "suhareke": 22, "suharekë": 22,
    "ferizaj": 23, "viti": 24, "vushtrri": 25, "zubin potok": 26, "zvecan": 27, "zveçan": 27,
    "malisheve": 28, "malishevë": 28, "novoberde": 29, "novobërdë": 29, "mitrovice": 30, "mitrovicë": 30,
    "junik": 31, "hani i elezit": 32, "mamushe": 33, "mamushë": 33, "gracanice": 34, "graçanicë": 34,
    "ranillug": 35, "partesh": 36, "kllokot": 37, "tirane": 38, "tirana": 38, "durres": 39, "durrës": 39,
    "shkoder": 40, "shkodër": 40, "elbasan": 41, "vlore": 42, "vlorë": 42, "korce": 43, "korçë": 43,
    "fier": 44, "berat": 45, "lushnje": 46, "pogradec": 47, "kavaje": 48, "kavajë": 48, "lac": 49, "laç": 49,
    "lezhe": 50, "lezhë": 50, "kukes": 51, "kukës": 51, "gjirokaster": 52, "gjirokastër": 52, "patos": 53,
    "kruje": 54, "krujë": 54, "kucove": 55, "kuçovë": 55, "sarande": 56, "sarandë": 56, "peshkopi": 57,
    "burrel": 58, "cerrik": 59, "corovode": 60, "çorovodë": 60, "shijak": 61, "librazhd": 62, "tepelene": 63,
    "gramsh": 64, "polican": 65, "bulqize": 66, "bulqizë": 66, "permet": 67, "përmet": 67, "fushe-kruje": 68,
    "fushë-krujë": 68, "kamez": 69, "kamëz": 69, "rreshen": 70, "ballsh": 71, "mamurras": 72, "bajram curri": 73,
    "erseke": 74, "ersëkë": 74, "peqin": 75, "divjake": 76, "divjakë": 76, "selenice": 77, "selenicë": 77,
    "bilisht": 78, "roskovec": 79, "kelcyre": 80, "kelcyrë": 80, "puke": 81, "pukë": 81, "memaliaj": 82,
    "rrogozhine": 83, "rrogozhinë": 83, "himare": 84, "himarë": 84, "delvine": 85, "delvinë": 85, "vore": 86, "vorë": 86,
    "koplik": 87, "maliq": 88, "perrenjas": 89, "shtermeni": 90, "krume": 91, "krumë": 91, "libohove": 92,
    "libohovë": 92, "orikum": 93, "fushe-arrez": 94, "fushë-arrëz": 94, "shengjin": 95, "shëngjin": 95, "rubik": 96,
    "milot": 97, "leskovik": 98, "konispol": 99, "kraste": 100, "kerrabe": 101, "berove": 102, "dellceva": 103,
    "dellçeva": 103, "kocani": 104, "koçani": 104, "kamenica": 105, "peceva": 106, "peçeva": 106, "probishtip": 107,
    "shtip": 108, "vinica": 109, "kratove": 110, "kriva pallanke": 111, "kumanove": 112, "manastir": 113,
    "demir hisari": 114, "krusheva": 115, "prilep": 116, "resnje": 117, "gostivar": 118, "tetove": 119, "tetovë": 119,
    "shkup": 120, "bogdance": 121, "gjevgjeli": 122, "radovishte": 123, "strumica": 124, "vallandova": 125,
    "diber": 126, "dibër": 126, "kercove": 127, "kërçove": 127, "brodi": 128, "oher": 129, "struge": 130, "strugë": 130,
    "demir kapia": 131, "kavadari": 132, "negotine": 133, "sveti nikolle": 134, "veles": 135, "komoran": 137,
    "bitola": 138, "golaj/has": 139, "dhermi": 140,
}
...
def process_order(order: dict):
    ...
    phone_clean = normalize_phone_number_digits_only(phone_raw)  # <- digits only (no +)
    ...
    if not token:
        ...
        return _email_vendors(order, vendor_items, address, city_id, phone_clean, first_name, last_name,
    ...
    for vendor, items in vendor_items.items():
        ...
        payload = {
            ...
            "Address": address,
            "IDCity": str(city_id),
            "Phone": phone_clean or "",  # digits only, per your request
            "Amount": f"{amount_for_courier:.2f}",
            "Comment": f"Shopify Order ID: {order.get('id','UNKNOWN')} | {'PREPAID' if prepaid else 'COD'}",
            "ShipmentPackage": 1,
            "CanOpen": False,
        ...
        if ok or ALWAYS_EMAIL:
            email_info = _email_one_vendor(order, vendor, items, first_name, last_name, address,
                                           phone_clean, city_id, cheetah_result,
...
def _email_one_vendor(order, vendor, items, first_name, last_name, address, phone, city_id, cheetah_result,
    ...
    # Build lines for text + HTML (with variant details & line totals)
    lines_txt = []
    rows_html = []
    for it in items:
        sku = it.get("sku") or ""
        ...
        # optional image (if present); many webhooks don't include it
        image_url = it.get("image") or (it.get("properties") or {}).get("image_url")
        ...
        img_html = f'<img src="{image_url}" alt="{safe(title)}" style="max-width:80px;max-height:80px;border:1px solid #ddd;border-radius:4px;">' if image_url else ""
        rows_html.append(f"""
          <tr>
            <td style="padding:8px;border:1px solid #eee;vertical-align:top">{img_html}</td>
